...
export default function MiniWebPreview({ brief, sections, note, fill=false, mobile=false, highlight=null }) {
  ...
  const TopNav = () => (
    <div data-prev-sec="nav" style={{
      ...
      {navLogoPos==="center" ? (
        <>
          {/* Logo v strede — hamburger a ikony napravo */}
          <div style={{ position:"absolute", left:"0.6rem", display:"flex", gap:"0.35rem", alignItems:"center" }}>
            {navSocials && <SocialDots/>}
          </div>
          <Logo/>
  ...
  const SideNav = () => (
    <div data-prev-sec="nav" style={{
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"0.35rem", alignItems:"center", flex:1 }}>
        {secs.slice(0, 5).map((s, i) => (
          <div key={i} title={s.label} style={{ width:26, height:4, borderRadius:2, background: i===0 ? primary : border }}/>
        ))}
      </div>
