...
//  Vizuálny náhľad webu z briefu: brand farby + fonty + sekcie
//  + VYBRANÉ FUNKCIE: hamburger, socials, carousel, dark mode,
//  jazyk, search, cookies, scroll-top, chatbot…
//  + NAVIGÁCIA podľa briefu:
//    · umiestnenie: horné / plávajúce (pod hero, ukotví sa) / ľavé / pravé
//    · správanie pri scrolle: ukotvené / statické / skrývajúce sa
//  Props:
//    fill      — vyplní rodiča na 100 % výšky (rámiky zariadení)
//    mobile    — mobilné zobrazenie (hamburger, 1-stĺpcové gridy)
//    highlight — id sekcie zvýraznenej prerušovanou čiarou + auto-scroll
// ═══════════════════════════════════════════════════════════════
...
const OH_SHORT = [["mon","Po"],["tue","Ut"],["wed","St"],["thu","Št"],["fri","Pi"],["sat","So"],["sun","Ne"]];
...
export default function MiniWebPreview({ brief, sections, note, fill=false, mobile=false, highlight=null }) {
  // Svetlosť pozadia brandu → farba zvýraznenia viditeľná v tmavej aj svetlej téme
  const hexLum = (h) => {
  ...
  const lightTheme = hexLum((brief.brand||{}).bg) > 0.5;
  const hlColor = lightTheme ? "#e85d00" : "#ff9540";

  // Jemné zvýraznenie práve zakliknutej sekcie — prerušovaná čiara podľa témy
  const hl = (id) => id && highlight===id
    ? { outline:`1.5px dashed ${hlColor}`, outlineOffset:-3, borderRadius:4, background:`${hlColor}0d` }
    : {};
  ...
  const navBehavior = brief.navBehavior || "sticky";
  // Bočné a plávajúce menu — na mobile vždy horné (hamburger)
  const navLayout   = mobile ? "top" : (brief.navLayout || "top");
  const sideNav     = navLayout === "left" || navLayout === "right";
  const floatingNav = navLayout === "floating" && hasHero;
  // Prepínač témy — pri sekcii darkmode ALEBO nastavení „Prepínač témy: áno" v Navigácii & Téme
  const darkToggle = has("darkmode") || brief.themeToggle === "yes";
  ...
  // Auto-scroll náhľadu na zvýraznenú časť (head / hero / obsah / footer)
  // lastHl guard — scrolluje len pri ZMENE zvýraznenia, nie pri každom re-renderi
  const scrollRef = useRef(null);
  const lastHl = useRef(null);
  useEffect(() => {
    if (!highlight) { lastHl.current = null; return; }
    if (highlight === lastHl.current || !scrollRef.current) return;
    lastHl.current = highlight;
    const el = scrollRef.current.querySelector(`[data-prev-sec="${highlight}"]`);
    if (el) {
  ...
  // Skrývajúce sa menu (hidden) — schová sa pri scrolle dole, zobrazí pri scrolle hore
  const [navHidden, setNavHidden] = useState(false);
  const lastScroll = useRef(0);
  useEffect(() => {
    if (navBehavior !== "hidden" || !scrollRef.current) return;
    const box = scrollRef.current;
    const onScroll = () => {
      const y = box.scrollTop;
      setNavHidden(y > 30 && y > lastScroll.current);
  ...
  const SocialDots = ({ vertical=false }) => (
    <span style={{ display:"inline-flex", gap:"0.25rem", alignItems:"center", flexDirection:vertical?"column":"row" }}>
      {["f","𝕏","in"].map(g => (
        <span key={g} style={{
    ...
    <div style={{ display:"flex", gap:"0.4rem", alignItems:"center" }}>
      ...
      {!mobile && (
        <span style={{ fontSize:"0.5rem", fontWeight:700, color:"#fff", background:primary,
          padding:"0.15rem 0.45rem", borderRadius:4, whiteSpace:"nowrap" }}>{brief.navCta || "Kontakt"}</span>
      )}
      {hamburger && (
  ...
  // ── HORNÉ / PLÁVAJÚCE MENU ────────────────────────────────
  // sticky = ukotvené; static = scrolluje s obsahom; hidden = schová sa pri scrolle dole
  const TopNav = () => (
    <div data-prev-sec="nav" style={{
      ...
      // Plávajúce menu sa VŽDY ukotví o vrch pri scrollovaní (sticky),
      // inak platí zvolené správanie pri scrolle
      position: (floatingNav || navBehavior!=="static") ? "sticky" : "relative",
      top:0, background:bg, zIndex:2,
      transform: !floatingNav && navBehavior==="hidden" && navHidden ? "translateY(-110%)" : "none",
      transition:"transform .3s ease",
      ...(floatingNav ? { borderTop:`1px solid ${border}` } : {}),
      ...hl("nav"),
    }}>
      {navLogoPos==="center" ? (
        <>
          {/* Logo v strede — položky menu vľavo, hamburger a ikony napravo */}
          <div style={{ position:"absolute", left:"0.6rem", display:"flex", gap:"0.4rem", alignItems:"center" }}>
            {!hamburger && navItems.slice(0,2).map((n,i)=>(
              <span key={i} style={{ fontSize:"0.5rem", color:muted, whiteSpace:"nowrap" }}>{n}</span>
            ))}
            {navSocials && <SocialDots/>}
          </div>
          ...
          <div style={{ position:"absolute", right:"0.6rem", display:"flex", gap:"0.35rem", alignItems:"center" }}>
            {!hamburger && navItems.slice(2,4).map((n,i)=>(
              <span key={i} style={{ fontSize:"0.5rem", color:muted, whiteSpace:"nowrap" }}>{n}</span>
            ))}
            {darkToggle && <span style={{ fontSize:"0.42rem", background:surface, border:`1px solid ${border}`, borderRadius:8, padding:"0.08rem 0.28rem" }}>🌙</span>}
            <span style={{ fontSize:"0.6rem", color:text, fontWeight:800, lineHeight:1 }}>☰</span>
          </div>
        </>
      ) : (
  ...
  // ── BOČNÉ MENU (ľavé / pravé) ─────────────────────────────
  const SideNav = () => (
    <div data-prev-sec="nav" style={{
      width:66, flexShrink:0, alignSelf:"stretch",
      [navLayout==="left" ? "borderRight" : "borderLeft"]:`1px solid ${border}`,
      background:surface, padding:"0.6rem 0.35rem",
      display:"flex", flexDirection:"column", alignItems:"center", gap:"0.55rem",
      position:"sticky", top:0, maxHeight:"100%",
      ...hl("nav"),
    }}>
      <div style={{ fontFamily:fd, fontWeight:800, fontSize:"0.5rem", color:text, textAlign:"center",
        overflow:"hidden", textOverflow:"ellipsis", maxWidth:"100%", whiteSpace:"nowrap" }}>
        {(brief.projectName || "Logo").slice(0, 8)}
      </div>
      <span style={{ fontSize:"0.6rem", color:text, fontWeight:800 }}>☰</span>
      {/* Položky menu — názvy sekcií */}
      <div style={{ display:"flex", flexDirection:"column", gap:"0.3rem", alignItems:"stretch", flex:1, width:"100%", minWidth:0 }}>
        {secs.slice(0, 6).map((s, i) => (
          <span key={i} title={s.label} style={{
            fontSize:"0.4rem", fontWeight: i===0 ? 700 : 500,
            color: i===0 ? primary : muted, textAlign:"center",
            whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
            borderLeft:`2px solid ${i===0 ? primary : "transparent"}`,
            paddingLeft:2, lineHeight:1.6,
          }}>{s.label}</span>
        ))}
      </div>
      {searchIcon && <span style={{ fontSize:"0.45rem", color:muted }}>🔍</span>}
      {darkToggle && <span style={{ fontSize:"0.4rem" }}>🌙</span>}
      {navSocials && <SocialDots vertical/>}
    </div>
  );
  ...
  const HeroBlock = () => hasHero ? (
    <div data-prev-sec="hero" style={{ padding: mobile ? "1.1rem 0.7rem 1rem" : "1.4rem 0.9rem 1.2rem", textAlign:"center",
      background:`radial-gradient(ellipse at 50% 0%, ${primary}22, transparent 65%)`, ...hl("hero") }}>
      {/* Pri plávajúcom menu je logo v hero (menu je až pod ním) */}
      {floatingNav && (
        <div style={{ fontFamily:fd, fontWeight:800, fontSize:"0.6rem", color:text, marginBottom:"0.5rem" }}>
          {brief.projectName || "Logo"}
        </div>
      )}
  ...
  // ── SEKCIE ────────────────────────────────────────────────
  const SectionBlocks = () => secs.map(s => (
    <div key={s.id} data-prev-sec={s.id} style={{ padding:"0.8rem 0.9rem", borderTop:`1px solid ${border}`, ...hl(s.id) }}>
      ...
      ) : s.id === "openinghours" && brief.openingHours?.days ? (
        <div style={{ maxWidth:220, marginInline:"auto" }}>
          {OH_SHORT.map(([k, lbl]) => {
            const d = brief.openingHours.days[k] || {};
            return (
              <div key={k} style={{ display:"flex", justifyContent:"space-between", gap:"0.5rem",
                fontSize:"0.48rem", color:d.open?text:muted, padding:"0.08rem 0",
                borderBottom:`1px solid ${border}` }}>
                <span style={{ fontWeight:700 }}>{lbl}</span>
                <span>
                  {d.open
                    ? `${d.from||"–"}–${d.to||"–"}${d.brk?` (prestávka ${d.brkFrom||""}–${d.brkTo||""})`:""}`
                    : "Zatvorené"}
                </span>
              </div>
            );
          })}
          <div style={{ fontSize:"0.44rem", color:brief.openingHours.holidaysOpen?primary:muted, marginTop:"0.25rem", textAlign:"center" }}>
            {brief.openingHours.holidaysOpen ? "🎉 Otvorené aj počas sviatkov" : "Počas sviatkov zatvorené"}
          </div>
        </div>
  ...
  const FooterBlock = () => hasFooter ? (
    <div data-prev-sec="footer" style={{ padding:"0.7rem 0.9rem", borderTop:`1px solid ${border}`, background:surface, ...hl("footer") }}>
      ...
      {/* Právne linky — súhlas so spracovaním údajov + cookies */}
      <div style={{ display:"flex", gap:"0.6rem", marginTop:"0.4rem", paddingTop:"0.35rem", borderTop:`1px solid ${border}`, flexWrap:"wrap" }}>
        <span style={{ fontSize:"0.42rem", color:muted, textDecoration:"underline" }}>Ochrana osobných údajov</span>
        <span style={{ fontSize:"0.42rem", color:muted, textDecoration:"underline" }}>Cookies</span>
        <span style={{ fontSize:"0.42rem", color:muted, textDecoration:"underline" }}>Nastavenia cookies</span>
      </div>
    </div>
  ) : null;

  // ── SKLADANIE STRÁNKY podľa umiestnenia menu ──────────────
  const pageContent = sideNav ? (
    // Bočné menu — sidebar vľavo/vpravo, obsah vedľa
    <div style={{ display:"flex", flexDirection: navLayout==="left" ? "row" : "row-reverse", alignItems:"stretch", minHeight:"100%" }}>
      <SideNav/>
    ...
    <>
      {/* Plávajúce menu — hero hore, menu pod ním; sticky ho ukotví pri scrolle */}
      {floatingNav && <HeroBlock/>}
      <TopNav/>
      {!floatingNav && <HeroBlock/>}
      <SectionBlocks/>
      <FooterBlock/>
  ...
  const pageInner = (
    <div ref={scrollRef} style={{ background:bg, fontFamily:fb, position:"relative",
