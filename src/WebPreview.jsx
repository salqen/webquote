...
export default function MiniWebPreview({ brief, sections, note, fill=false, mobile=false, highlight=null }) {
  // Jemné zvýraznenie naposledy zakliknutej sekcie — svetlo oranžová prerušovaná čiara
  const hl = (id) => id && highlight===id
    ? { outline:"1.5px dashed #ff9540", outlineOffset:-3, borderRadius:4 }
    : {};
  ...
  const pageInner = (
    <div style={{ background:bg, fontFamily:fb, position:"relative",
      ...
      {/* Sekcie */}
      {secs.map(s => (
        <div key={s.id} style={{ padding:"0.8rem 0.9rem", borderTop:`1px solid ${border}`, ...hl(s.id) }}>
      ...
      {/* Footer */}
      {hasFooter && (
        <div style={{ padding:"0.7rem 0.9rem", borderTop:`1px solid ${border}`, background:surface, ...hl("footer") }}>
