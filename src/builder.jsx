...
// ── SLIDERY pre hero carousel (public/sliders — z priečinka SLIDERS) ──
const SLIDER_OPTIONS = [
  { id:"fade",      label:"Fade / Cross-dissolve", file:"slider-1-fade.html",      desc:"Jemné prelínanie slidov" },
  { id:"slide",     label:"Slide",                 file:"slider-2-slide.html",     desc:"Klasický horizontálny posun" },
  { id:"coverflow", label:"Coverflow",             file:"slider-3-coverflow.html", desc:"3D coverflow efekt (Apple štýl)" },
  { id:"kenburns",  label:"Ken Burns",             file:"slider-4-kenburns.html",  desc:"Pomalý zoom a posun obrazu" },
  { id:"cube",      label:"Cube",                  file:"slider-5-cube.html",      desc:"3D rotácia kocky" },
  { id:"split",     label:"Split",                 file:"slider-6-split.html",     desc:"Rozdelený prechod na polovice" },
  { id:"circle",    label:"Circle reveal",         file:"slider-7-circle.html",    desc:"Kruhové odhalenie slidu" },
  { id:"cards",     label:"Cards / Stack",         file:"slider-8-cards.html",     desc:"Karty ukladané na seba" },
];
                                    ...
                                    {SLIDER_OPTIONS.map(sl=>{
                                      ...
                                      return (
                                        <button key={sl.id} onClick={()=>update({heroSlider: sel ? "" : sl.id})} style={{
                                          ...
                                          <div style={{position:"relative", aspectRatio:"16/7", overflow:"hidden", background:"#0c0c0f"}}>
                                            <iframe
                                              src={`/sliders/${sl.file}`}
                                              title={sl.label}
                                              loading="lazy"
                                            ...
                                            />
                                            {sel && (
                                              <div style={{position:"absolute",top:6,right:6,background:c.pri,color:"#fff",
                                                borderRadius:"50%",width:20,height:20,display:"flex",alignItems:"center",
                                                justifyContent:"center",fontSize:"0.7rem",fontWeight:800}}>✓</div>
                                            )}
                                          </div>
                                          <div style={{padding:"0.4rem 0.55rem"}}>
                                            <div style={{fontSize:"0.7rem",fontWeight:sel?700:600,color:sel?c.pri:c.text}}>{sl.label}</div>
                                            <div style={{fontSize:"0.58rem",color:c.desc,lineHeight:1.3}}>{sl.desc}</div>
                                          </div>
                                        </button>
                                      );
                                    })}
