...
export function generateProPrompt(b, cat) {
  ...
  // Google / Bing Maps place
  if (b.gmapsStatus === "have" && clean(b.gmapsUrl)) identityLines.push(`- **Google Maps place:** ${clean(b.gmapsUrl)} — link it from the map/contact section.`);
  if (b.gmapsStatus === "need") identityLines.push(`- **Google Maps place:** client NEEDS one created (use the business identity & address above).`);
  if (b.bingStatus === "have" && clean(b.bingUrl)) identityLines.push(`- **Bing Maps place:** ${clean(b.bingUrl)}`);
  if (b.bingStatus === "need") identityLines.push(`- **Bing Maps place:** client NEEDS one created (use the business identity & address above).`);

  // Otváracie hodiny — pre sekciu Opening hours, footer a schema.org
  const OH_EN = { mon:"Monday", tue:"Tuesday", wed:"Wednesday", thu:"Thursday", fri:"Friday", sat:"Saturday", sun:"Sunday" };
  const ohDays = b.openingHours?.days || null;
  const ohLines = ohDays ? Object.entries(OH_EN).map(([k, name]) => {
    const d = ohDays[k] || {};
    if (!d.open) return `  - ${name}: closed`;
    return `  - ${name}: ${d.from || "?"}–${d.to || "?"}${d.brk ? ` (break ${d.brkFrom || "?"}–${d.brkTo || "?"})` : ""}`;
  }) : [];
  if (ohLines.length) {
    identityLines.push(`- **Opening hours (use in the Opening-hours section, footer & schema.org openingHoursSpecification):**\n${ohLines.join("\n")}\n  - Public holidays: ${b.openingHours?.holidaysOpen ? "OPEN" : "closed"}`);
  }
